...
const UserList = ({ activeChannel }) => {
    ...
    if (isError) {
        return <div className="team-channel-list__message">Fail to load users</div>;
    }
            ...
            {users.map((user) => {
                ...
                return (
                    <button
                        ...
                        className={`str-chat__channel-preview-messenger ${isActive && "!bg-black/20 !hover:ng-black/20 border-l-8 border-purple-500 shadow-lg0"}`}
                    >
                        <div className="flex items-center gap-2 w-full">
                            <div className="relative">
                                {user.image ? (
                                    <img src={user.image} alt={user.name || user.id} className="size-4 rounded-full" />
                                ) : (
                                    <div className="size-4 rounded-full bg-gray-400 flex items-center justify-center">
