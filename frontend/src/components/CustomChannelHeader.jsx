...
const CustomChannelHeader = () => {
    ...
    const handleVideoCall = async () => {
        if (channel) {
            const calUrl = `${window.location.origin}/call/${channel.id}`;
            await channel.sendMessage({
                text: `I've started a video call, join me here: ${calUrl}`
            })
        }
    }
    return (
        <div className='h-14 border-b border-gray-200 flex items-center px-4 justify-between  bg-white'>
            ...
            <div className="flex items-center gap-3">
                <button className="flex items-center gap-2 hover:bg-[#F8F8F8] py-1 px-2 rounded cursor-pointer"
                ...
                <button button className="hover:bg-[#F8F8F8] p-1 rounded cursor-pointer"
                    onClick={handleVideoCall} title="Start Video Call">
                ...
                <button className="hover:bg-[#F8F8F8] p-1 rounded  cursor-pointer" onClick={handleShowPinned}>
